...
const Summary = () => {
  ...
  const validTimeData = summary.emissionsOverTime
    .filter(
      (item) =>
        item.month && item.day && typeof item.totalEmission === "number"
    )
    .map((item) => ({
      x: new Date(2025, item.month - 1, item.day),
  ...
  const emissionByTypeData = {
    labels: summary.breakdownByType.map((item) =>
  ...
  return (
    <div className="bg-[#eff1e6] min-h-screen w-full">
      ...
      <div className="w-full h-full relative px-4 pb-[5rem] sm:pb-0 sm:px-6  pt-[4.5rem]">
        <div className="bg-gradient-to-br from-[#87bc71] to-[#447c62] rounded-xl shadow-md p-4 sm:p-6 mb-6 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div
            ...
            className={`bg-[#bfe2b5] p-4 sm:p-6 rounded-xl w-full transition-transform duration-500 ${
              hovered ? "scale-[1.04]" : "scale-[0.96]"
            }`}
          >
            <h2 className="text-lg font-semibold mb-4">Emission by Type</h2>
            {summary.breakdownByType.length === 0 ? (
              <p className="text-center text-gray-600">
                No activity submitted yet.
              </p>
            ) : (
              <div className="w-full max-w-[400px] h-auto aspect-square mx-auto transition-transform duration-500 hover:scale-[1.04]">
          ...
          <div
            className={`bg-[#bfe2b5] p-4 sm:p-6 rounded-xl transition-transform duration-500 ${
            ...
            {summary.emissionsOverTime.length === 0 ? (
              ...
              <div className="w-full overflow-x-auto">
                <Line
                  ...
                  options={emissionOverTimeOptions}
